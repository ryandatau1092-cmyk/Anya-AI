...
const CallView: React.FC<CallViewProps> = ({ config, onEndCall }) => {
          ...
          callbacks: {
            ...
            onerror: (e: any) => { 
              ...
              if (msg.includes('429')) {
                ...
                setErrorMessage('Kuota Gratis Habis (429). Tunggu sejenak.');
              } else {
                setStatus('ERROR');
  ...
  return (
    <div className="relative w-full h-[100dvh] flex flex-col items-center py-4 md:py-8 px-4 md:px-12 text-white animate-in fade-in duration-700 overflow-hidden">
      <div 
      ...
      <div 
        ...
        style={{ 
          ...
          background: `radial-gradient(circle at center, rgba(236,72,153,${agentActivity/70}) 0%, transparent 80%)`,
          filter: `blur(${40 + agentActivity/2}px)`,
          transform: `translate(-50%, -50%) scale(${1 + agentActivity/50})`
        }}
      />
      ...
      <div className="text-center w-full max-w-screen-lg relative z-20 flex flex-col items-center gap-1 shrink-0">
        <div className="flex items-center justify-center gap-2">
          <div className={`w-1.5 h-1.5 rounded-full animate-pulse shadow-[0_0_8px] ${errorMessage ? 'bg-red-500 shadow-red-500/50' : 'bg-green-500 shadow-green-500/50'}`}></div>
          <p className="text-[9px] md:text-xs font-black tracking-[0.2em] opacity-40 uppercase">
            Live • {formatTime(timer)}
          </p>
        </div>
        <h1 className="text-2xl md:text-4xl font-black tracking-tighter truncate max-w-[95%] mx-auto drop-shadow-xl">
          {config.name}
        </h1>
        <div className="flex flex-col items-center gap-1.5 mt-1">
          ...
          {errorMessage && (
            <div className="flex flex-col items-center gap-1.5">
              <p className="text-[8px] md:text-[10px] text-white font-bold bg-red-600/60 backdrop-blur-lg px-3 py-1 rounded-full uppercase border border-white/10">{errorMessage}</p>
              {!isQuotaError && <button onClick={handleReconnect} className="text-[8px] font-black underline uppercase text-white/40">Reconnect</button>}
            </div>
          )}
        </div>
      </div>
      ...
      {/* CENTER SECTION */}
      <div className="w-full max-w-screen-lg flex-1 flex flex-col items-center justify-center gap-6 md:gap-10 z-10 overflow-hidden px-4 relative min-h-0">
        <div className="relative shrink-0">
          <div className={`relative w-28 h-28 sm:w-40 sm:h-40 md:w-56 md:h-56 rounded-[35px] md:rounded-[60px] overflow-hidden border-2 md:border-4 shadow-2xl transition-all duration-300 z-10 ${agentActivity > 5 ? 'border-pink-500 scale-[1.05]' : 'border-white/10'}`}>
            <img 
              src={config.profilePic || ''} 
              className="w-full h-full object-cover" 
              style={{ transform: `scale(${1 + agentActivity/200})` }}
              alt="Profile" 
            />
            {isFinishingSpeech && <div className="absolute inset-0 bg-pink-500/10 backdrop-blur-[1px] animate-pulse"></div>}
          </div>
        </div>

        <div className="w-full flex flex-col gap-4 items-center max-w-2xl shrink-0">
          <div className="w-full bg-white/5 backdrop-blur-3xl rounded-[30px] border border-white/10 p-5 md:p-8 transition-all shadow-xl min-h-[80px] md:min-h-[140px] flex items-center justify-center overflow-y-auto custom-scrollbar">
             {agentSpeechText ? (
               <p className="text-center text-sm md:text-2xl font-black text-white leading-tight animate-in fade-in slide-in-from-bottom-2 duration-500 drop-shadow-[0_2px_10px_rgba(236,72,153,0.3)]">
                  "{agentSpeechText}"
               </p>
             ) : (
               <p className="text-center text-[9px] md:text-sm font-black text-white/20 italic tracking-widest uppercase">
                 {transcription || (isMuted ? "Mikrofon Mati" : "Dengerin lo...")}
               </p>
             )}
          </div>
          <div className="w-full h-1.5 md:h-2 bg-white/5 rounded-full overflow-hidden">
             <div 
               className="h-full bg-gradient-to-r from-pink-500 via-purple-500 to-pink-500 bg-[length:200%_100%] animate-gradient transition-all duration-75 ease-out shadow-[0_0_15px_rgba(236,72,153,0.5)]" 
               style={{ width: `${Math.min(100, agentActivity * 3 || (isFinishingSpeech ? 100 : 0))}%` }}
             />
          </div>
        </div>
      </div>
      ...
      {/* BOTTOM SECTION */}
      <div className="w-full max-w-screen-md flex flex-col items-center gap-4 md:gap-6 z-20 shrink-0 pb-6 md:pb-10">
        <div className="flex items-center gap-4 md:gap-6">
          <button 
            onClick={() => setIsMuted(!isMuted)}
            className={`p-4 md:p-6 rounded-[25px] md:rounded-[35px] transition-all border border-white/10 active:scale-90 ${isMuted ? 'bg-red-500 text-white shadow-xl' : 'bg-white/5 text-white/50 hover:bg-white/10'}`}
          >
          ...
          <button 
            ...
            className="p-6 md:p-10 bg-red-600 hover:bg-red-500 rounded-full shadow-[0_15px_45px_rgba(220,38,38,0.4)] transition-all hover:scale-105 active:scale-95 text-white border-4 border-white/5"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 md:h-10 md:w-10 rotate-[135deg]" viewBox="0 0 20 20" fill="currentColor">
              <path d="M2 3a1 1 0 011-1h2.153a1 1 0 01.986.836l.74 4.435a1 1 0 01-.54 1.06l-1.548.773a11.037 11.037 0 006.105 6.105l.774-1.548a1 1 0 011.059-.54l4.435.74a1 1 0 01.836.986V17a1 1 0 01-1 1h-2C7.82 18 2 12.18 2 5V3z" />
            </svg>
          </button>
          <button 
            onClick={() => setShowSettings(true)}
            className="p-4 md:p-6 bg-white/5 hover:bg-white/10 rounded-[25px] md:rounded-[35px] text-white/50 hover:text-white transition-all border border-white/10 active:scale-90"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 md:h-6 md:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="w-full flex items-center gap-3 bg-white/5 border border-white/10 rounded-full p-1.5 pl-5 focus-within:ring-2 ring-pink-500/40 transition-all shadow-xl backdrop-blur-3xl group/input">
          <input 
            type="text" 
            placeholder="Bisikin sesuatu..." 
            className="flex-1 bg-transparent outline-none py-2.5 md:py-3.5 text-xs md:text-sm font-bold text-white placeholder:text-white/20"
            value={inputText}
          ...
          <button 
            ...
            disabled={!inputText.trim() || status === 'THINKING...' || isQuotaError}
            className="p-3 md:p-4 bg-gradient-to-br from-pink-500 to-pink-600 rounded-full text-white transition-all disabled:opacity-20 active:scale-90 shadow-lg group-hover/input:scale-105"
          >
      ...
      {showSettings && (
        <div className="absolute inset-0 z-[110] flex items-center justify-center p-6 animate-in fade-in duration-300">
           ...
           <div className="relative w-full max-w-md bg-zinc-900/90 border border-white/10 rounded-[40px] p-8 shadow-2xl backdrop-blur-3xl max-h-[85dvh] overflow-y-auto custom-scrollbar">
              <header className="flex justify-between items-center mb-6">
                 <h3 className="text-lg font-black uppercase tracking-[0.2em] text-pink-500">Config</h3>
                 <button onClick={() => setShowSettings(false)} className="p-2 hover:bg-white/5 rounded-full text-white/40">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                   ...
                   <select 
                      className="w-full bg-black border border-white/10 rounded-2xl px-5 py-4 font-bold text-xs md:text-sm text-white outline-none appearance-none"
                      value={selectedMicId}
              ...
              <button 
                ...
                className="w-full mt-10 bg-gradient-to-r from-pink-500 via-purple-600 to-pink-500 bg-[length:200%_auto] hover:bg-right text-white font-black py-5 rounded-[30px] uppercase text-[10px] tracking-[0.4em] shadow-xl transition-all"
              >
                Save & Close
              </button>
           </div>
